...
URL = ['https://mhl.khl.ru/stat/leaders/860/bls/' ]
...
def get_content(html):
    ...
    #statistics_db = []
    table_name = 'mhl_play_off_blocked_throws'  # это название таблицы, куда пихаем (не бд)
    fields_name = ['place', 'name', 'number','club', 'roll','played_games', 'scored_goals', 'assists', 'points',
                   'plus_minus','blocked_throws', 'power_tricks'  ]  # и так далее. Это список самих полей
    ...
    for statistic in statistics:
        ...
        club = player[4]
        if club:
            club = club.get_text().replace('\n' and '\t', '')
        statistics_db = [
            player[0].get_text(),
            player[2].find('a').get_text(),
            player[3].get_text(),
            club,
            player[5].get_text(),
            player[6].get_text(),
